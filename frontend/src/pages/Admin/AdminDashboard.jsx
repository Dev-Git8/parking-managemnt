        ...
        <div className="min-h-screen bg-brand-light-bg dark:bg-brand-dark flex transition-all duration-300">
            ...
            <aside className="w-80 border-r border-slate-100 dark:border-brand-dark-card bg-white dark:bg-brand-dark sticky top-0 h-screen p-8 flex flex-col justify-between">
                <div className="space-y-12">
                     <div className="flex items-center space-x-3 px-2">
                        ...
                        <span className="text-xl font-black text-brand-black dark:text-white tracking-tighter uppercase italic">Oversight.SYS</span>
                    </div>

                    <div className="space-y-2">
                        <SidebarItem icon={Building2} label="Partner Network" id="businesses" />
                        <SidebarItem icon={Users} label="User Directory" id="users" />
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div className="bg-slate-50 dark:bg-brand-dark-card p-6 rounded-3xl border border-slate-100 dark:border-white/5 space-y-4">
                        <div className="flex items-center space-x-3">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Root Access: Confirmed</span>
                        </div>
                        <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest leading-relaxed">System administrator mode active. Total encryption enabled.</p>
                    </div>
            ...
            <main className="flex-1 p-12 overflow-y-auto">
                <header className="flex items-center justify-between mb-16">
                    <div>
                        <h1 className="text-4xl font-black text-brand-black dark:text-white tracking-tighter uppercase mb-2">Central Control</h1>
                        <p className="text-slate-400 dark:text-slate-500 font-bold text-xs uppercase tracking-[0.3em]">Operational Oversight Node</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                     {[
                         { label: 'Network Identity Nodes', value: stats.users, icon: Users, color: 'text-indigo-500', bg: 'bg-indigo-500/10' },
                         { label: 'Validated Partners', value: stats.businesses, icon: Building2, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
                         { label: 'Verification Queue', value: stats.pending, icon: Clock, color: 'text-amber-500', bg: 'bg-amber-500/10', alert: stats.pending > 0 }
                     ].map((stat, i) => (
                ...
                <div className="bg-white dark:bg-brand-dark-card rounded-[3rem] border border-slate-50 dark:border-white/5 shadow-premium overflow-hidden">
                    <div className="p-8 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
                         <h3 className="text-2xl font-black text-brand-black dark:text-white tracking-tight uppercase">Database: {activeTab}</h3>
                         <div className="flex items-center space-x-4">
                              <div className="relative">
                                 <SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={16} />
                                 <input placeholder="Filter identity..." className="bg-slate-50 dark:bg-brand-dark border-none py-2.5 pl-10 pr-6 rounded-xl text-xs font-bold text-brand-black dark:text-white focus:ring-2 focus:ring-brand-yellow/20" />
                              </div>
                              <Button variant="secondary" size="sm">System Dump</Button>
                         </div>
                    </div>
                        ...
                        {activeTab === 'businesses' ? (
                            <table className="w-full text-left">
                                <thead className="bg-slate-50 dark:bg-brand-dark">
                                    <tr>
                                        <th className="px-10 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Partner Details</th>
                                        <th className="px-10 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Metric Config</th>
                                        <th className="px-10 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-center">Protocol Status</th>
                                        <th className="px-10 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-right">Admin Logic</th>
                                    </tr>
                                </thead>
                                        ...
                                        <tr key={biz.id} className="group hover:bg-slate-50/50 dark:hover:bg-brand-dark transition-colors">
                                            ...
                                            <td className="px-10 py-8">
                                                <div className="text-lg font-black text-brand-black dark:text-white font-outfit">
                                                    ${biz.price_per_hour} <span className="text-[10px] text-slate-400 uppercase ml-1">Logic Unit</span>
                                                </div>
                                                <div className="text-slate-500 dark:text-slate-500 text-[10px] font-bold mt-1 uppercase tracking-widest">OWNER_ID: {biz.owner_name}</div>
                                            </td>
                            ...
                            <table className="w-full text-left">
                                <thead className="bg-slate-50 dark:bg-brand-dark">
                                    <tr>
                                        <th className="px-10 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Network Identity</th>
                                        <th className="px-10 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Access Key</th>
                                        <th className="px-10 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Entry Matrix</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-50 dark:divide-white/5">
                                    {users.map((u) => (
                                        <tr key={u.id} className="group hover:bg-slate-50/50 dark:hover:bg-brand-dark transition-colors">
                                            ...
                                            <td className="px-10 py-8 text-slate-400 dark:text-slate-500 text-xs font-bold font-outfit italic">
                                                {new Date(u.created_at).toLocaleDateString()} SYSTEM_LOG_IN
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
